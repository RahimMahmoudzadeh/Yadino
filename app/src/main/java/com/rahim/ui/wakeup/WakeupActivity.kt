...
class WakeupActivity : ComponentActivity() {
  private var routineModel: RoutineModel? = null
          ...
          Column(
            ...
            Text(
              ...
              modifier = Modifier
                ...
                .fillMaxWidth(),
              color = Color.White,
              text = resources.getString(R.string.forget_work, routineModel?.name),
            )
            Column {
...
//                                alarmManagement.cancelAlarm(this@WakeupActivity, alarmId)
  ...
  private fun getIntentResult() {
    routineModel = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
      intent.getParcelableExtra(Constants.ROUTINE, RoutineModel::class.java)
    } else {
